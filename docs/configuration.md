...
```json
{
  ...
  // The identity information of the node, which can be read and generated using the
  // `host -peerkey ./peer.key` command and saved in the `./peer.key` file. Back it up and do not delete it.
  // The following is just an example. Please generate and fill it out by yourself during actual deployment.
  // PeerID is generated by algorithm using public key, and the program will verify whether PeerID and PrivKey match.
  "Identity": {
    // Node ID of the node, which is a unique ID that identifies different nodes in a distributed communication network.
  ...
  // The list of AI projects supported by the node, which can be managed using the registration/unregistration
  // interface, but manual modification is not recommended.
  "AIProjects": [
    {
      // Project name
      "Project": "SuperImage",
